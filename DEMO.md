...
| **1:30–3:00** | **dApp** → Connect wallet → "Get test dUSDC" → **Link income (Plaid Sandbox)** → **Submit income proof**. | The attester (the enclave, in production) signs the income attestation; the `attest_income` tx runs **on-chain `verify_signature` → TRUE → a non-transferable `IncomeAttestation` appears** (income $4,200). *Raw bank data never touched the chain.* Then hit **Tamper**: flip one signature byte, resubmit → **tx ABORTS (`E_BAD_SIGNATURE`)**. Accept-good / reject-bad, live — the whole thesis. |
...
- **Say the honest line about the key (it reads as maturity, not weakness):** on testnet the enclave key is bound via the documented **operator fallback** (`register_via_operator`); the native **`register_via_nitro`** (real AWS attestation, COSE + X.509 to the AWS root CA in the framework, **PCR-pinned**) is proven in `contracts/tests/nitro_tests.move` and is the *one call* that flips this to a live Marlin Oyster enclave on mainnet. Same `verify_signature` gate either way.
- **Production-ready:** the pool is generic over the coin type, so mainnet swaps dUSDC → native USDC unchanged; Plaid Sandbox → production is a host+key swap (already whitelisted, no PCR churn).
