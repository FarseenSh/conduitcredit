...
- **Real Sui-native work, live:** `attest_income` calls the real `verify_signature` (Ed25519 over a BCS `IntentMessage`) against a registered `Enclave`; a forged signature aborts on chain (testnet digest `HZEeVqB8…`).
